# The disk and the bootloader, for an arm64 Hetzner Cloud VM.
#
# # UEFI only, which is the reason this file is not the x86 one
#
# `machines/base` carries a BIOS boot partition AND an ESP, because an x86
# Hetzner instance may be either and nothing in a configuration can ask. Its
# comment records what that cost to learn: a UEFI-only GRUB there produces
# `### Done! ###`, a correctly written disk, a provider reporting the server
# running, and a machine that never boots. Two went that way.
#
# arm64 has no such ambiguity. Hetzner's CAX line is UEFI, so the 1 MB EF02
# partition and the MBR install are answering a question this hardware does not
# ask, and carrying them would be copying a scar rather than the lesson.
#
# That difference is the whole argument for two templates instead of one with a
# conditional. A template is a thing somebody FORKS, and a fork full of branches
# for hardware they do not own is worse than a short file for the machine they
# have. What it costs is real and is stated here rather than discovered: every
# other module in this directory is a copy of `machines/base`, and a fix to one
# has to be made in both. `fetch_template` copies a single subdirectory, so a
# template cannot reference shared files outside its own root.
#
...
{ lib, pkgs, ... }:
{
  boot.loader.grub = {
    ...
    # No MBR here, unlike the x86 template: there is no BIOS path to install to
    # on this hardware, and naming a device would ask GRUB to write a boot
    # sector nothing will ever read.
    devices = [ "nodev" ];
  };
  ...
  disko.devices.disk.main = {
    ...
    content = {
      type = "gpt";
      partitions = {
        # No EF02 partition. That exists on the x86 template so a BIOS machine
        # has somewhere to put GRUB's core image on a GPT disk; this hardware
        # boots UEFI and reads the ESP below.
        #
